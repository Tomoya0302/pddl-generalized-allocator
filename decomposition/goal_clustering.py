...
def cluster_goals_into_subtasks(
    ...
    # 3.5 サブタスク数の上限が指定されている場合は、クラスタをマージして減らす
    if max_num_subtasks is not None and len(final_clusters) > max_num_subtasks:
        # 末尾のクラスタを順に前のクラスタへマージして、数を減らす素朴な戦略
        while len(final_clusters) > max_num_subtasks:
            tail = final_clusters.pop()        # 最後のクラスタ
            # 残っている中で一番後ろのクラスタにマージ
            final_clusters[-1].extend(tail)
